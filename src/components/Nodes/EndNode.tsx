...
export const EndNode: React.FC<NodeProps<EndWorkflowNode>> = ({ id, data, selected }) => {
  ...
  return (
    <BaseNode id={id} type="end" title={data.title || 'End'} icon={<CircleStop size={15} />} selected={selected} accentColor="#ef4444" accentBg="bg-rose-50/80" validationError={getNodeError(id)}>
      {data.endMessage ? (
